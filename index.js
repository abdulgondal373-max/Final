...
app.get('/', (req, res) => res.send('Bot AKH en mode auto !'));
app.listen(port);
...
client.once('ready', () => {
    client.user.setActivity('DM pour avoir le lien', { type: ActivityType.Watching });
    console.log("Le bot AKH est prêt !");
});

client.on('messageCreate', async (message) => {
    if (message.author.bot) return;

    // COMMANDE PRIVÉE : !dire [ID] [Message]
    ...
    // RÉPONSE AUTO EN DM
    if (!message.guild) {
        console.log(`Message pour AKH de ${message.author.tag} | ID: ${message.author.id}`);
